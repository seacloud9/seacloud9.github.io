...
init3JS = function () {
  ...
  jQuery(window).on('mousewheel', function (event) {
    ...
    for (var si = 0; si < soundPlaneDiv.length; si++) {
      new TWEEN.Tween(soundPlaneDiv[si].position).to({ z: soundPlaneDiv[si].position.z + event.deltaY * 20 }, 500).easing(TWEEN.Easing.Exponential.Out).start();
    }
  });
  buildPlanes();
};
...
ngSoundCloudService.service('playIDService', function () {
  ...
  'soundCloudService',
  'playIDService',
  function AppCtrl($scope, $location, soundCloudService, playIDService) {
