...
class UIManager {
  ...
  constructor() {
    ...
    // Social share buttons (optional - removed from UI)
    if (this.screenshotElShareX) {
      this.screenshotElShareX.addEventListener('click', this._shareScreenshotX)
    }
    if (this.screenshotElShareFb) {
    ...
    if (!Settings.touch) {
      ...
      // Social share buttons hover (optional - removed from UI)
      if (this.screenshotElShareX) {
  ...
  _modeBtnClick = (e) => {
    ...
    if (data === MODE.GAME) {
      ...
      // Resume mode: Show navigation menu for interactive resume
      console.log('Activating RESUME mode - showing navigation menu')
      this.joystickEl.classList.remove('is-game')
      this.jumpBtnEl.classList.remove('is-game')
      ...
      if (resumeNav) {
        console.log('Showing resume navigation menu')
        resumeNav.classList.remove('hidden')
      } else {
        console.error('Resume navigation element not found!')
      }

      // Show social footer
      const socialFooter = document.querySelector('.social-footer')
      if (socialFooter) {
        console.log('Showing social footer')
        socialFooter.classList.remove('hidden')
      }

      // Auto-sail to About island after mode selection
      setTimeout(() => {
        console.log('Triggering auto-sail to About island')
        ResumeNavigationManager.navigateToIsland(0) // About island
      }, 1000)
    }
