...
export default class ParticlesSide {
  ...
  constructor(parent, debug) {
    ...
    // Safety check: If particles object doesn't exist in the model, skip initialization
    if (!particlesEdgeMesh || !particlesEdgeMesh.geometry) {
      console.warn('Particles object not found in boat model - particle effects disabled')
  ...
  update({ time, delta, turnForce, velocity }) {
    // Skip update if particles weren't initialized
