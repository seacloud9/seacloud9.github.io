...
export default class Splashes {
  ...
  constructor(parent) {
    ...
    // Safety check: If splash objects don't exist in the model, skip initialization
    if (!this.#mesh1 || !this.#mesh1Parent || !this.#mesh2 || !this.#mesh2Parent) {
      console.warn('Splash objects not found in boat model - splash effects disabled')
  ...
  update({ time, delta }) {
    // Skip update if splash objects weren't initialized
  ...
  transitioningSpeed(mode) {
    // Skip if splash objects weren't initialized
    if (!this.#isInitialized) return
